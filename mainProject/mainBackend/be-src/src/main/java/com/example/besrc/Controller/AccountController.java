...
public class AccountController {
    ...
    @Operation(summary = "Update my account information", responses = {
            @ApiResponse(responseCode = "200", description = "Success", content = @Content(mediaType = "application/json",schema = @Schema(implementation = AccountResponse.class))),
            @ApiResponse(responseCode = "401", description = "Invalid", content = @Content(mediaType = "application/json", schema = @Schema(implementation = AccountResponse.class))),
            @ApiResponse(responseCode = "403", description = "Forbidden", content = @Content(mediaType = "application/json", schema = @Schema(implementation = AccountResponse.class)))
    }, parameters = {
            @Parameter(name = "Authoriztion", in = ParameterIn.HEADER, schema = @Schema(type = "string"), example = "Bearer <token>", required = true)
    })
    public ResponseEntity<MyApiResponse> updateProfile(
            @RequestBody @Valid AccountUpdateRequest request,
            @AuthenticationPrincipal UserDetails userDetails  // Lấy user từ JWT
    ) {
        String username = userDetails.getUsername();  // Lấy username từ token
        MyApiResponse response = accountService.updateProfile(username, request);
