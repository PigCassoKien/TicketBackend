...
public class HallController {
    ...
    @Operation(summary = "Get all seats by hall ID", responses = {
            @ApiResponse(responseCode = "200", description = "Get Seats Successfully", content = @Content(mediaType = "application/json", schema = @Schema(implementation = Seat.class))),
            @ApiResponse(responseCode = "404", description = "Hall not found", content = @Content(mediaType = "application/json", schema = @Schema(implementation = MyApiResponse.class)))
    })
    @PreAuthorize("hasAnyAuthority('ROLE_ADMIN', 'ROLE_SUPER_ADMIN')")
    public ResponseEntity<List<Seat>> getAllSeatsByHall(@PathVariable("hallId") String hallId) {
        List<Seat> seats = hallService.getAllSeatsByHall(hallId);
